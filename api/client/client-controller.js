...
const createClient = async (req, res) => {
  try {
    ...
    if (req.body.id) {
      return res.status(400).json({ msg: "Something went wrong. Please try again!", isError: true });
    }

    const clientList = await Client.findAll({ where: { email_id: clientEmail } });
    if (clientList.length > 0) {
      return res.status(409).json({ msg: "Email already exists", isError: true });
    }

    const userList = await User.findAll({ where: { email: clientEmail } });
    if (userList.length > 0) {
      return res.status(409).json({ msg: "User already exists!", isError: true });
    }
