...
const updateWorkExperience = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const [updatedRows] = await WorkExperience.update({ ...workExp, ...filePaths }, {
        where: { candidate_id: workExp.candidate_id, id: workExp.id },
      });
...
const deleteWorkExperience = async (req, res) => {
  try {
    ...
    if (workExperienceToDelete) {
      await Promise.all([
        workExperienceToDelete.salarySlip && fs.unlink(path.join(__dirname, "..", "..", workExperienceToDelete.salarySlip)),
        workExperienceToDelete.relievingLetter && fs.unlink(path.join(__dirname, "..", "..", workExperienceToDelete.relievingLetter)),
        workExperienceToDelete.experienceLetter && fs.unlink(path.join(__dirname, "..", "..", workExperienceToDelete.experienceLetter))
      ]);
    }
